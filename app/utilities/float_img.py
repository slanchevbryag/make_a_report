'''
Этот файл позволяет создать плавающую картинку
для добавления её в документ.
PS. Код вставила, но пока не могу его прочитать (сложно),
поэтому оставила комментарии автора.
Взято отсюда
https://docs-python.ru/packages/modul-python-docx-python/rabota-risunkami/
'''
...
class ANCHOR(BaseOxmlElement):
    ...
    @classmethod
    def new(cls, cx, cy, shape_id, pic, pos_x: int, pos_y: int):
        """
...
def new_pic_anchor(part, image_descriptor, width: int, height: int, pos_x: int, pos_y: int) -> str:
    """
    ...
    return ANCHOR.new_pic_anchor(
        shape_id, rid, filename, cx, cy, pos_x, pos_y)


# смотрите: docx.text.run.add_picture
def add_float_picture(
        img: Any, image_path_or_stream: str,
        width: int = None, height: int = None, pos_x: int = 0, pos_y: int = 0) -> None:
    """
